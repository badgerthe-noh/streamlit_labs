...
# 1. 페이지 기본 설정 ===============================
st.set_page_config(
    page_title='COVID-19 Korea DashBoard',
    page_icon='🦠',
    layout='wide' # 전체 화면 너비 사용
)

st.title('KR COVID-19 Korea Patient DashBoard')

# 2. 파일 업로더 =====================================
#       파일이 업로드 되지 않았다면 None 반환
uploaded_confirmed = st.file_uploader('Upload csv file of infection confirmed', type=['csv'])
uploaded_deaths = st.file_uploader('Upload csv file of death', type=['csv'])
uploaded_recovered = st.file_uploader('Upload csv file of recovered patient', type=['csv'])

# 3. 세 파일이 모두 업로드 되었을 때만 분석이 실행되도록===========
#       업로드 전 : 파일 객체가 None -> False -> 분석 실행 안함 -> 다시 업로드 유도
#       업로드 후 : 파일 객체가 있음  -> True  -> 분석 실행 가능
if uploaded_confirmed and  uploaded_deaths and uploaded_recovered:
    ...
    # .diff();현재 빼기 이전 --> fillna(0);정수 0으로 채우기 --> .astype(int);int로 변환
    ...
    # 10. 탭 UI 구성
    tab1, tab2, tab3 = st.tabs(['Infection Pattern', 'Stat summary', 'Proportion Analysis'])

    # 11. tab1 : Infection Pattern
    with tab1: 
        # ① 누적 추이 (선 그래프) ─────────────────────────────
        st.subheader('Cum pattern graph')
...
# 파일 미업로드 시 상태 안내 메시지
    ...
    st.info('Upload all 3 csv files(infected, dead, recovered)!')

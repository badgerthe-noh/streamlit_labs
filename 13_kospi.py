...
# 8. 주식 데이터 불러오는(로드) 함수 정의
def get_stock_data(code, start, end):
    """
    함수 설명: FinanceDataReader로 개별 종목의 주가 데이터 조회
    ...
    try:   # 오류가 날 가망성이 있는 코드 # 오류나면 except로
        df=fdr.DataReader(code, start, end)
        if df.empty:
            return None
        return df
    except Exception as e: # e-> 예외메시지를 의미
        st.error(f'{code} 데이터 로드 실패: {e}')
        return None
    
# 9. 현재가 / 전일 대비 변동폭 카드 표시
for i, code in enumerate(codes): # i는 인덱스 번호까지 리스트에 나오게끔
    df = get_stock_data(
        code,
        start_date.strftime('%Y-%m-%d'),
        end_date.strftime('%Y-%m-%d')
    )
    if df is not None and len(df) >= 2: # 전 날, 전전 날의 데이터가 있어야 하니까
        current = df['Close'].iloc[-1] # 가장 최근
        ...
        st.metric(
            ...
            value=f'{current:,}원',
            delta=f'{delta:,}원' # 천단위 구분 기호 ,
        )
    else:
        st.warning(f'{selected_stocks[i]} 데이터가 충분하지 않습니다!')
        
# 10. 탭 기능 : 라인 차트 / 캔들 스틱 차트
    ...
    if len(codes) == 1: # 종목을 하나만 선택했다면
        ...
        for code in codes: 
            ...
            if df is not None: # 결과가 있다면
                df_temp = df[['Close']].rename(columns={'Close':code}) # df_temp는 임시 df 
                dfs.append(df_temp)
        if dfs: # 병합할 데이터가 있다면
            merged_df = pd.concat(dfs, axis=1) #axis=1은 가로로 붙일 때(수평 병합)
    ...
    for i, code in enumerate(codes): # i는 인덱스 번호까지 리스트에 나오게끔
        ...
        if df is not None: # df가 있다면
            ...
            fig_candle.update_layout(
                title=f'{selected_stocks[i]} 캔들 스틱 차트(최근 3개월)',
                xaxis_title='날짜',
                yaxis_title='가격(원)',
